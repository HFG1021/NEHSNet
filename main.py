...
from model.Model_NEHSNet import Model
...
def train(dataloader, model, model_refine, optimizer, epoch):
    ...
    for i, data in enumerate(tqdm(dataloader, 0)):
        ...
        if args.refine:
            ...
            loss = eval_cal.mpjpe(output_3D, out_target)+0.5*eval_cal.p_mpjpe(output_3D, out_target)
        else:
            loss = eval_cal.mpjpe(output_3D, out_target)+0.5*eval_cal.p_mpjpe(output_3D, out_target)
...
if __name__ == '__main__':
    ...
    model = Model(depth=args.depth, d_model=args.d_model, n_heads=args.n_heads, dropout=args.dropout).cuda()
